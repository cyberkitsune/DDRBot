...
def generate_embed(score_data, score_player):
    ...
    total_notes = int(score_data.score_marv_count.value) + int(score_data.score_perfect_count.value) + int(score_data.score_great_count.value) + int(
        score_data.score_good_count.value) + \
                  int(score_data.score_OK_count.value) + int(score_data.score_miss_count.value)

    marv_percent = int((int(score_data.score_marv_count.value) / total_notes) * 100)
    perfect_percent = int((int(score_data.score_perfect_count.value) / total_notes) * 100)
    great_percent = int((int(score_data.score_great_count.value) / total_notes) * 100)
    good_percent = int((int(score_data.score_good_count.value) / total_notes) * 100)
    OK_percent = int((int(score_data.score_OK_count.value) / total_notes) * 100)
    miss_percent = int((int(score_data.score_miss_count.value) / total_notes) * 100)
    ...
    emb.add_field(name="👀 Marvelous", value="%s (%i%%)" % (score_data.score_marv_count, marv_percent), inline=True)
    emb.add_field(name="👌 Perfect", value="%s (%i%%)" % (score_data.score_perfect_count, perfect_percent), inline=True)
    emb.add_field(name="👍 Great", value="%s (%i%%)" % (score_data.score_great_count, great_percent), inline=True)
    emb.add_field(name="😦 Good", value="%s (%i%%)" % (score_data.score_good_count, good_percent), inline=True)
    emb.add_field(name="<:emiok:572201794982248452> OK", value="%s (%i%%)" % (score_data.score_OK_count, OK_percent), inline=True)
    emb.add_field(name="<:eming:572201816792629267> Miss", value="%s (%i%%)" % (score_data.score_miss_count, miss_percent), inline=True)
...
def generate_embed_from_db(score_data, score_player, verified=False):
    ...
    total_notes = int(score_data.marv_count) + int(score_data.perf_count) + int(score_data.great_count) + int(score_data.good_count) + \
        int(score_data.OK_count) + int(score_data.miss_count)

    marv_percent = int((int(score_data.marv_count) / total_notes)*100)
    perfect_percent = int((int(score_data.perf_count) / total_notes) * 100)
    great_percent = int((int(score_data.great_count) / total_notes) * 100)
    good_percent = int((int(score_data.good_count) / total_notes) * 100)
    OK_percent = int((int(score_data.OK_count) / total_notes) * 100)
    miss_percent = int((int(score_data.miss_count) / total_notes) * 100)
    ...
    emb.add_field(name="👀 Marvelous", value="%s (%i%%)" % (score_data.marv_count, marv_percent), inline=True)
    emb.add_field(name="👌 Perfect", value="%s (%i%%)" % (score_data.perf_count, perfect_percent), inline=True)
    emb.add_field(name="👍 Great", value="%s (%i%%)" % (score_data.great_count, great_percent), inline=True)
    emb.add_field(name="😦 Good", value="%s (%i%%)" % (score_data.good_count, good_percent), inline=True)
    emb.add_field(name="<:emiok:572201794982248452> OK", value="%s (%i%%)" % (score_data.OK_count, OK_percent), inline=True)
    emb.add_field(name="<:eming:572201816792629267> Miss", value="%s (%i%%)" % (score_data.miss_count, miss_percent), inline=True)
